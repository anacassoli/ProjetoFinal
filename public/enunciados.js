...
async function handleSearch() {
  ...
  const questionsByVestibular = await fetchJson(
    `${apiBase}/buscar/vestibular/${encodeURIComponent(term)}`,
    {
      headers: getAuthHeaders()
    }
  );
  if (questionsByVestibular) {
    renderTable(questionsByVestibular);
  }
  ...
  const questionsByAno = await fetchJson(
